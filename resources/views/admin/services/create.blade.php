        ...
        <div class="flex items-center gap-4">
            ...
            <div class="text-left text-white">
                ...
                <p class="text-gray-300 text-xs sm:text-sm mt-0.5">
                    Tambahkan jenis jasa atau layanan konsultasi untuk ditampilkan di website publik.
                </p>
            </div>
        </div>
    </div>
        ...
        <form action="{{ route('services.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <textarea name="deskripsi_singkat"
                          rows="3"
                          class="w-full px-4 py-3.5 bg-gray-50/50 border border-gray-200 rounded-xl text-sm focus:bg-white focus:border-[#C79A3B] focus:ring-4 focus:ring-[#C79A3B]/10 transition"
                          placeholder="Tulis ringkasan singkat layanan ini...">{{ old('deskripsi_singkat') }}</textarea>
            ...
            <div>
                ...
                <textarea name="deskripsi_lengkap"
                          rows="5"
                          class="w-full px-4 py-3.5 bg-gray-50/50 border border-gray-200 rounded-xl text-sm focus:bg-white focus:border-[#C79A3B] focus:ring-4 focus:ring-[#C79A3B]/10 transition"
                          placeholder="Tulis penjelasan mendetail mengenai layanan...">{{ old('deskripsi_lengkap') }}</textarea>
            ...
            <div>
                <label class="block text-[11px] font-bold uppercase tracking-wider text-[#3B2508] mb-2">
                    Foto / Dokumentasi Layanan <span class="text-gray-400 font-normal">(Opsional)</span>
                </label>
                <input type="file"
                       name="gambar"
                       accept="image/*"
