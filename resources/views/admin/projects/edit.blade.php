        ...
        <div class="flex items-center gap-4">
            ...
            <div class="text-left text-white">
                ...
                <p class="text-gray-300 text-xs sm:text-sm mt-0.5">
                    Perbarui data pengalaman pekerjaan CV Sahabat Eksplorasi Banua.
                </p>
            </div>
        </div>
    </div>
        ...
        <form action="{{ route('projects.update', $project->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <textarea name="jenis_pekerjaan"
                          rows="4"
                          class="w-full px-4 py-3.5 bg-gray-50/50 border border-gray-200 rounded-xl text-sm focus:bg-white focus:border-[#C79A3B] focus:ring-4 focus:ring-[#C79A3B]/10 transition"
                          placeholder="Contoh: Eksplorasi, FS, AMDAL, Kajian Teknis, dll.">{{ old('jenis_pekerjaan', $project->jenis_pekerjaan) }}</textarea>
            ...
            <div>
                ...
                <textarea name="deskripsi"
                          rows="3"
                          class="w-full px-4 py-3.5 bg-gray-50/50 border border-gray-200 rounded-xl text-sm focus:bg-white focus:border-[#C79A3B] focus:ring-4 focus:ring-[#C79A3B]/10 transition"
                          placeholder="Tambahkan informasi pendukung pekerjaan...">{{ old('deskripsi', $project->deskripsi) }}</textarea>
                @error('deskripsi')
                    <p class="text-red-500 text-xs mt-1.5 font-medium flex items-center gap-1">
                        <svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                        {{ $message }}
            ...
            @if($project->gambar)
                <div>
                    ...
                    <div class="overflow-hidden rounded-2xl border border-gray-200 inline-block shadow-sm">
                        <img src="{{ asset('storage/'.$project->gambar) }}" class="w-64 h-44 object-cover">
                    </div>
                </div>
            @endif
            ...
            <div>
                ...
                <p class="text-xs text-gray-400 mt-1.5">
                    Kosongkan jika tidak ingin mengganti dokumentasi lama. Format JPG, JPEG, PNG maksimal 2MB.
                </p>
                @error('gambar')
                    <p class="text-red-500 text-xs mt-1.5 font-medium flex items-center gap-1">
                        <svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
